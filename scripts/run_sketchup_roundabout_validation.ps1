...
$handoff = Join-Path $projectRoot 'output\sketchup_validation_v029_roundabout\roundabout_validation.ptsu.json'
$pluginMain = Join-Path $projectRoot 'test_artifacts\sketchup_runtime_v029_roundabout\plugin\planning_toolbox_sketchup\main.rb'
$baseValidationDir = Join-Path $projectRoot 'test_artifacts\sketchup_runtime_v029_roundabout'
...
foreach ($requiredPath in @($sketchup, $runner, $bootstrap, $handoff, $pluginMain)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Missing SketchUp roundabout validation file: $requiredPath"
    }
}

if (Test-Path -LiteralPath $report -PathType Leaf) {
    $runStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
    $validationDir = Join-Path $projectRoot "test_artifacts\sketchup_runtime_v029_roundabout_rerun_$runStamp"
...
if (-not (Test-Path -LiteralPath $report -PathType Leaf)) {
    throw "SketchUp roundabout validation report was not written in time. PID=$($process.Id)"
}
