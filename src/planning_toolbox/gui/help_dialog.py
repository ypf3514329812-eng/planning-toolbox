...
class HelpDialog(QDialog):
    ...
    def _workflows_html() -> str:
        ...
        <ol><li>先选择一张包含有效闭合 PARCEL 地块的 DXF，并等待检查完成。通过“4. GIS 导出与导入”导入的面会自动写入 PARCEL 图层，可直接接续本页。</li><li>打开“6. 方案草图生成”。</li><li>先选择“规范依据框架”：居住区项目可选择居住区国家标准框架，公共/民用建筑可选择民用建筑国家标准框架；正式项目仍应优先选择自定义/地方条件。</li><li>在“布局风格”中选择“自然曲线（推荐）”，系统会生成圆角建筑轮廓和弧形通行引导；需要规整示意时可切换为“简洁矩形”。</li><li>填写每个地块的建筑数量、概念建筑覆盖率、建筑退线距离；如需控制建筑之间的距离，再填写“概念建筑间距”。</li><li>如需生成弧形道路/消防引导，填写“概念道路/消防通道宽度”；建筑和停车位会尽量避开该通道。</li><li>如果希望估算总建筑面积，填写明确的楼层数；如果希望估算停车位，再填写“停车配比（个/1000m²）”。</li><li>确认单位可识别，再点击运行。</li><li>在输出目录打开 *_concept_plan.dxf。它会保留原始图纸，并增加 CONCEPT_SETBACK、CONCEPT_BUILDING、CONCEPT_PARKING、CONCEPT_GREEN、CONCEPT_ROAD、CONCEPT_LABEL 和 CONCEPT_DIMENSION 图层；同时生成可用 Excel 打开的 *_concept_plan_schedule.csv。</li></ol>
        ...
        <div class="green"><strong>在系统内接受或拒绝候选：</strong>图片转 CAD 完成后，点击结果区的“复核候选对象”。左侧是候选表，右侧显示完整 CAD；点选一行后会用红框定位。可以多选后批量“接受”“拒绝”或“恢复待确认”，误操作点“撤销上一步”。只有点击“保存复核结果”才写入轻量语义文件；取消不会保存。接受项继续按建筑、道路、绿化等用途传给 SketchUp；拒绝项不会从 DXF 删除，只会降级为锁定参考底图，因此以后仍可重新打开并恢复。</div>
        ...
        <ol><li>先准备一张俯视、正交、尽量没有透视和阴影的平面效果图。彩色分区图使用低饱和度红/灰/绿/蓝/米色；线稿既可以白底黑线，也可以黑底白线。</li><li>打开“9. AI 效果图转 CAD”，选择 PNG/JPG 图片，再选择“彩色分区 CAD”或“黑白线稿 CAD”。黑白模式默认自动判断底色，判断不正确时可手动指定。</li><li>填写图片中整个场地的实际宽度，例如图片场地宽 100 米；系统不会替你猜测比例。</li><li>黑白线稿建议选择“精细”，并保持“自动整理线条”开启。系统会连接方向一致的短段，保护正放或旋转建筑候选，把重复小圆形符号归并为 PT_TREE 块，把至少三个尺寸与方向相近的窄矩形归并为 PT_PARKING_STALL 停车位块，并把规则圆形/椭圆拟合为可编辑 ELLIPSE 候选。</li><li>先看彩色 CAD 预览：暖红建筑、浅灰道路、绿色树木、米黄停车位是否与底图基本重合；仍为浅灰线的对象表示系统尚未可靠确认用途。</li><li>点击“复核候选对象”，按右侧红框定位结果，批量接受明显正确项、拒绝明显错误项；不确定的继续保留“待确认”。</li><li>查看“道路中心线候选”中的安全拼接数量、路口安全吸附和橙/红复核叠加图；橙色高可信短段可保守连接，红色低可信线不会自动生成 SU 道路实体。</li><li>保持“使用已确认的精修 CAD 知识辅助校正”开启，并明确选择图纸类型。系统不会把“待确认”类型和不同类型项目混在一起。</li><li>保持“生成轻量 Markdown 图纸知识卡”开启并填写少量检索标签。知识卡只保存原图指纹、比例依据、识别摘要、复核状态和成果路径，不保存图片像素。</li><li>通常不要勾选“收藏本次 DXF 为候选 CAD 样本”。候选样本只供查看，不会用于校正，以免错误结果污染知识库。</li><li>点击运行并先查看预览。结果中的“知识库辅助”会显示匹配了多少份精修 CAD、实际校正了多少个规则图元。</li><li>需要进一步精修时，再在 CAD 中隔离 BW_BUILDING_CANDIDATE、BW_ROAD_CENTERLINE_CANDIDATE、BW_TREE_CANDIDATE、BW_PARKING_CANDIDATE 和 BW_LANDSCAPE_CANDIDATE；若只想看线稿，直接关闭 `BW_BUILDING_FILL` 和 `BW_ROAD_FILL`，边界线不会被删除。</li><li>需要进一步清理时，再运行“8. 图纸质量增强检查 → 最低人工修改”，最后在 CAD 中复核比例、闭合、道路连接和复杂曲线。</li><li>完成真正的人工精修后，回到结果区点击“⭐ 收藏精修 CAD”，选择精修 DXF 并确认。系统提取建筑、停车位和树木的米制尺寸；下次使用相同输出知识库、选择相同图纸类型时才会参与校正。</li></ol>
        ...
        <div class="green"><strong>看懂“质量关卡”：</strong>任务运行完成不等于成果已经可以提交。结果区会另行显示三种结论：“概念流程可继续”表示没有发现阻断项；“需人工复核”表示可以继续做概念方案，但应先检查列出的道路、轮廓、比例或模型项；“已阻断”表示源文件校验、尺度或交接关系有问题，应先修复，不能直接进入下一步。每次图片转 CAD 和 CAD 转 SketchUp 都会生成一份“中文质量复核清单”，在下方生成文件中双击即可打开，按“优先操作”逐项检查。JSON 文件只用于机器追溯，普通使用者不需要编辑。</div>
