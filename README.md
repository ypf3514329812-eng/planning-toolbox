...
- 结果区可打开“候选对象复核”，在原生 CAD 预览上定位候选，并批量接受、拒绝、恢复待确认或撤销本轮操作；取消窗口不会写入。决定只保存到轻量 sidecar，原 DXF 继续通过 SHA-256 零破坏校验。被拒绝对象在后续 SketchUp 交接中降为参考底图，明确接受的道路中心线可进入道路建模。
...
每次图片转 CAD 和 CAD 转 SketchUp 还会新增一个轻量 `*_quality_baseline.json`。它汇总来源 SHA-256、明确比例、语义交接、道路连通、斑马线方向、建筑/道路覆盖和课程模型复核项；结果始终以“可追溯证据 + 待复核项”呈现，不把概念图转化或程序化建模误说成测绘精度、审批结论或课程评分。可将不同方案的该文件并排查看，以定位返工主要来自底图比例、道路、候选语义还是模型覆盖。

`v0.61.0` 将候选复核接入主界面和 CAD→SketchUp 数据链。复核表支持多选、只选待确认、接受、拒绝、恢复待确认和有限步撤销；选中对象会在 CAD 预览中以红框定位。保存时校验 sidecar 版本并原子写入，避免覆盖其他会话的新决定；被拒绝对象保留为可追溯底图而不参与自动建模，明确接受的低置信度道路中心线可覆盖“仅可信道路”自动门槛。拆分、合并和逐顶点修形仍需在 CAD 中完成。
